Shader"Custom/Grass" {
	Properties{
		...
		_MainTex("Albedo (RGB)", 2D) = "white" {} //主要显示的材质颜色
		_AlphaTex("Alpha (A)", 2D) = "white" {} //遮罩图
		_Height("Grass Height", float) = 3 //生长高度
		_Width("Grass Width", range(0, 0.5)) = 0.05 //宽度

	}
	SubShader{
		//渲染状态设置，应用于这个subshader的所有pass
		//这里表示不运用背面剔除效果，将会渲染这个对象的双面材质
		Cull off
			//这里使用的Tag含义
			//Queue=AlphaTest  绘制所有实体物体后，渲染
			//RenderType=TransparentCutout 通常的植被会使用到,例如如果是使用的Terrain对象，里面刷的植物就是这个
			//IgnoreProjector=True 其他投影之类的材质贴图不会影响到这片草
		...
		Pass
		{
			...
			//开启Geometry Shader编程语句，用于之后定制一些自己的函数
			...
			struct v2g
			{
				float4 pos : SV_POSITION;//像素点基于屏幕空间下的坐标数据
				float3 norm : NORMAL;//模型空间的法线方向
		...
		//上面#pragam声明的vert函数名，标识顶点数据用于这个函数 使用Unity内置的一个结构体appdata_full 传递这些数据给上面自定义的结构v2g
		...
		//创建由5个Quad组成的一棵草
		//每个Quad由2个三角面组成
		//2*3*5 = 30 输出30个顶点数 maxvertexcount(30)
		// [1]这个函数一次接受1个点数据v2g
		//inout标识输出TriangleStream  三角面
		[maxvertexcount(30)]
		void geom(point v2g points[1], inout TriangleStream<g2f> triStream)
		{
		 
			float4 root = points[0].pos;//拿到这个传入的点的坐标

			const int vertexCount = 12; //实时创建的草总共需要12个顶点
			...
			//类型为g2f的数组v 长度为 vertexCount
			//这个草的12个顶点数据，用到了上面定义的g2f结构体初始化函数
			g2f v[vertexCount] = {
			...
			//处理纹理坐标
			float currentV = 0;//初始的纹理高度，取0-1的0
			float offsetV = 1.f /((vertexCount / 2) - 1); //这张纹理图总共被分为了5片，每级纹理的颜色点偏移值增加 1/5
			...
			*/
			...
			for (int i = 0; i < vertexCount; i++)
			{
				...
				//这个顶点的索引是否是偶数索引
				//偶左基右
				if (fmod(i , 2) == 0) //是个偶数点，左边
				{ 
					//x=根位置-宽度, y=根位置+应该在的高度  ,  根的z轴位置
				...
				{ 
					...
					//然后下一个点应该就是在更高的位置，纹理高度+每级的纹理偏移值
					currentV += offsetV;
